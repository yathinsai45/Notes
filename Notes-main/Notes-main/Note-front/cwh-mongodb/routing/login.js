...
login.post("/login", [
    ...
    body("password", "enter valid password first").isLength({ min: 5 })
], async (req, res) => {
    try {
        let success = false;
        ...
        if (!errors.isEmpty()) {
            return res.status(200).json({success, message: errors.array() });
        }
        ...
        if (!there) {
            return res.json({success, message: "enter your correct email" });
        }
        ...
        if (!correctpassword) {
            return res.status(200).json({success, message: "enter correct password to login your account" });
        }

        // if all details are correct
        // res.json({ hurray: "you login to your account" });
        success=true;
        res.json({success,there});
    } catch {
